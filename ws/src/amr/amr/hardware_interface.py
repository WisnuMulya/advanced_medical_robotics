...
from rclpy.logging import (
    LoggingSeverity,
)  # Import LoggingSeverity for setting log levels
...
class HardwareInterfaceNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            "node is alive"
        )  # Log message indicating the node is running
        self.get_logger().set_level(
            LoggingSeverity.INFO
        )  # Increasing logging severity to avoid continuous printing to terminal
        ...
        self.portHandler = PortHandler(
            self.DEVICENAME
        )  # Port handler for communication
        self.packetHandler = PacketHandler(
            2.0
        )  # Packet handler for sending/receiving data
    ...
    def set_pos(self, id, pos):
        ...
        self.packetHandler.write4ByteTxRx(
            self.portHandler, id, self.ADDR_GOAL_POSTION, int(pos)
        )
    ...
    def set_vel(self, id, vel):
        ...
        self.packetHandler.write4ByteTxRx(
            self.portHandler, id, self.ADDR_GOAL_VELOCITY, int(vel)
        )
    ...
    def set_cur(self, id, cur):
        ...
        self.packetHandler.write2ByteTxRx(
            self.portHandler, id, self.ADDR_GOAL_CURRENT, int(cur)
        )
    ...
    def set_operating_mode(self, mode):
        ...
        for id in self.DXL_IDs:
            ...
            # Verify mode update
            mode_actual, _, _ = self.packetHandler.read1ByteTxRx(
                self.portHandler, id, self.ADDR_OPERATING_MODE
            )
        if mode_actual == mode:
    ...
    def desired_pos_callback(self, pos_msg):
        ...
        if len(targets) != len(self.DXL_IDs):
            self.get_logger().info(
                "Number of given angles doesn't match number of motors"
            )
            return
    ...
    def desired_vel_callback(self, vel_msg):
        ...
        if len(targets) != len(self.DXL_IDs):
            self.get_logger().info(
                "Number of given velocities doesn't match number of motors"
            )
            return
    ...
    def desired_cur_callback(self, cur_msg):
        ...
        if len(targets) != len(self.DXL_IDs):
            self.get_logger().info(
                "Number of given currents doesn't match number of motors"
            )
            return
    ...
    def joint_pos_rel_callback(self, pos_rel_msg):
        ...
        if len(rel_positions) != len(self.DXL_IDs):
            self.get_logger().info(
                "Mismatch in the number of joints and relative position commands"
            )
            return
    ...
    def get_vel(self, id):
        vel, _, _ = self.packetHandler.read4ByteTxRx(
            self.portHandler, id, self.ADDR_PRESENT_VELOCITY
        )
        return float(self.s16(vel)) * self.VEL_SCALING

    # Read the current current (amperage) of a motor
    def get_cur(self, id):
        cur, _, _ = self.packetHandler.read2ByteTxRx(
    ...
    def joint_state_callback(self):
        ...
        if (
            self.joint_pos_all[idx] >= self.LIMIT_POS
            or self.joint_pos_all[idx] <= -self.LIMIT_POS
        ):
            self.get_logger().info(
                f"Joint {id} at or beyond limit: Position = {self.joint_pos_all[idx]}"
            )
        ...
        if self.operating_mode == self.MODE_VEL:
            for idx, id in enumerate(self.DXL_IDs):
                predicted_pos = (
                    self.joint_pos_all[idx]
                    + self.timer_period * self.joint_vel_all[idx]
                )

                # Check if the predicted position exceeds the limits
                if (predicted_pos > (self.LIMIT_POS + self.limit_pos_tol)) & (
                    self.joint_vel_all[idx] > 0.0
                ):
                    self.set_vel(
                        id, 0.0
                    )  # Stop the joint to prevent exceeding the limit

                elif (predicted_pos < (-self.LIMIT_POS + self.limit_pos_tol)) & (
                    self.joint_vel_all[idx] < 0.0
                ):
                    self.set_vel(
                        id, 0.0
