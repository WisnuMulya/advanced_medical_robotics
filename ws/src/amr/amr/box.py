...
class box(Node):
    def __init__(self):
        """
        Create a Box node to act as the controller for the robot
        """
        super().__init__("box")
        ...
        # set constants
        self.k_spring = 40. # Spring constant
        self.c_damper = 4. # Damper constant
    ...
    def get_f_dir(self, pos):
        ...
        is_above_line_bottom = y > -self.box_length/2
        is_above_line_top = y > self.box_length/2
        is_left_right_line = x > self.box_centre - (self.box_length/2)
        is_left_left_line = x > self.box_centre + (self.box_length/2)
        f_dir = [0., 0., 0.]
        displacement = None

        if is_above_line_top:
            if is_left_left_line:
                f_dir = [-1., -1., 0.]
                displacement = np.linalg.norm(pos - np.array([self.box_centre + (self.box_length/2), self.box_length/2]))
            elif ~is_left_right_line:
                f_dir = [1., -1., 0.]
                displacement = np.linalg.norm(pos - np.array([self.box_centre - (self.box_length/2), self.box_length/2]))
            else:
                f_dir = [0., -1., 0.]
                displacement = y - (self.box_length/2)
        elif ~is_above_line_bottom:
            if is_left_left_line:
                f_dir = [-1., 1., 0.]
                displacement = np.linalg.norm(pos - np.array([self.box_centre + (self.box_length/2), -self.box_length/2]))
            elif ~is_left_right_line:
                ...
                f_dir = [0., 1., 0.]
                displacement = np.abs(y - (-self.box_length/2))
        else:
            if is_left_left_line:
                f_dir = [-1., 0., 0.]
    ...
    def joint_state_callback(self, msg):
        ...
        f_dir, displacement = self.get_f_dir(pos)
